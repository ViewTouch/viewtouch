...
/*********************************************************************
 * NOTE BAK-> All of the code in this file except BFEncrypt and
 *  BFDecrypt were taken from the Crypt::Blowfish Perl module.
 *  Specifically, I stole _blowfish.c and blowfish.h and reformatted
 *  them somewhat.
 ********************************************************************/
/* File: bf.c
...
BFkey_type  bfkey;  // the global key
...
/****
 * crypt_block: This function requires the block to be two 32 bit integers, in 
 ...
 ****/
static void crypt_block(IntU32 block[2], BFkey_type *my_bfkey, short direction)
{
    IntU32 left, right, (*S)[256], *P;

    left = block[0];
    right = block[1];

    S = my_bfkey->sbox;
    P = my_bfkey->p[direction];
...
/****
 * blowfish_crypt_8bytes: The following should be allignment and endian
 *   independent. I have not tested it on a little-endian machine.
 ...
 ****/
void blowfish_crypt_8bytes(IntU8 *source, IntU8 *dest, BFkey_type *my_bfkey, short direction)
{
    IntU32  block[2];
    ...
    crypt_block( block, my_bfkey, direction );
 ...
 ****/
int blowfish_make_bfkey(unsigned char *key_string, int keylength, BFkey_type *my_bfkey)
{
    int       i, j, k ;
    IntU32    dspace[2];
    IntU32    checksum=0 ;
    ...
    {
        my_bfkey->p[0][i] = p_init[i] ;
        my_bfkey->p[1][NROUNDS+1-i] = p_init[i] ;
    ...
    {
        strcpy((char *)my_bfkey, "Bad initialization data");
        return -1;
    }
    ...
    for( i=0 ; i<10 ; ++i )
        crypt_block( dspace, my_bfkey, 0 ) ;
    checksum = dspace[0] ;
    for( i=0 ; i<10 ; ++i )
        crypt_block( dspace, my_bfkey, 1 ) ;
    if( (checksum!=0xaafe4ebd) || dspace[0] || dspace[1] )
    {
		strcpy((char *)my_bfkey, "Error in crypt_block routine");
        return -1;
    }
    ...
    {
        crypt_block( dspace, my_bfkey, 0 ) ;
        my_bfkey->p[0][i] = dspace[0] ;
        my_bfkey->p[1][NROUNDS+1-i] = dspace[0] ;
        my_bfkey->p[0][i+1] = dspace[1] ;
        my_bfkey->p[1][NROUNDS-i] = dspace[1] ;
    }
    ...
    for ( i=0 ; i<4 ; ++i )
    {
        ...
        {
            crypt_block( dspace, my_bfkey, 0 ) ;
            my_bfkey->sbox[i][j] = dspace[0] ;
            my_bfkey->sbox[i][j+1] = dspace[1] ;
        }
    }
    return 0;
}
...
int  BFEncrypt(char *key, int keylen, char *ciphrtext, int ciphrlen, char *plaintext, int *plainlen)
{
    ...
    if (key != NULL)
        blowfish_make_bfkey((unsigned char *)key, keylen, &bfkey);

    padlen = (*plainlen + 1) % 8;
    if (padlen != 0)
        padlen = 8 - padlen;
    totallen = *plainlen + padlen + 1;
    blocks = totallen / 8;

    memset(ciphrtext, 0x00, ciphrlen);
    memset(buffer, 0x00, STRLONG);

    buffer[0] = padlen + 48;
    for (idx = 1; idx <= padlen; idx++)
        buffer[idx] = '0';  // maybe use a random number here?
    strcat(buffer, plaintext);

    block = buffer;
    ciphrblock = ciphrtext;
    for (idx = 0; idx < blocks; idx++)
    {
        blowfish_crypt_8bytes((IntU8 *)block, (IntU8 *)ciphrblock, &bfkey, 0);
        block += 8;
        ciphrblock += 8;
    }
    *plainlen = totallen;
...
int  BFDecrypt(char *key, int keylen, char *plaintext, int plainlen, char *ciphrtext, int *ciphrlen)
{
    ...
    memset(plaintext, 0x00, plainlen);
    memset(buffer, 0x00, STRLONG);
    ...
    for (idx = 0; idx < blocks; idx++)
    {
        blowfish_crypt_8bytes((IntU8 *)block, (IntU8 *)plainblock, &bfkey, 1);
        block += 8;
        plainblock += 8;
    }
    idx = 1;
    padlen = buffer[0] - 48;
    idx += padlen;
    strcpy(plaintext, &buffer[idx]);
    *ciphrlen = strlen(plaintext);
    return retval;
}
