...
public:
    ...
    Expense();
    Expense(int &no);
    int SetFileName(const char *buffer, int maxlen, const genericChar *path);
    int Read(InputDataFile &infile, int version);
    int Write(OutputDataFile &outfile, int version);
    int Load(const char *path);
    int Save(const char *path);
    int IsBlank();
    int Author(Terminal *term, const genericChar *employee_name);
    int DrawerOwner(Terminal *term, const genericChar *drawer_name, Archive *archive = NULL);
    int AccountName(Terminal *term, const genericChar *account_name, Archive *archive = NULL);
    int IsTraining();
