...
int ExpenseDB::MoveTo(ExpenseDB *exp_db, Drawer *DrawerList)
{
    ...
    while (currExpense != nullptr)
    {
        ...
        if (currExpense->drawer_id > -1)
        {
            ...
            if (currDrawer != nullptr && currDrawer->Status() == DRAWER_BALANCED)
            {
                move = 1;
            }
        }
 ...
 *   have been balanced, et al.  Giving exp_db as nullptr acts as a Purge()
 *   (MoveAll to /dev/null).
 ****/
...
Expense *ExpenseDB::FindByRecord(Terminal *term, int no, int drawer_type)
{
    ...
    while (thisexp != nullptr && count < maxcount)
    {
        ...
        if ((drawer == nullptr) || (StatusMatch(drawer_type, drawer->Status())))
        {
            if (count == no)
            {
                retexp = thisexp;
...
int ExpenseDB::FindRecordByWord(Terminal *term, const genericChar* word, int start, Archive *archive)
{
    ...
    while (thisexp != nullptr && count < maxcount)
    {
        if (dlist != nullptr)
            drawer = dlist->FindBySerial(thisexp->drawer_id);
        if ((drawer == nullptr) || (StatusMatch(DRAWER_OPEN, drawer->Status())))
        {
