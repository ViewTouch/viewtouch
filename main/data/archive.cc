...
int Archive::LoadPacked(Settings *settings, const char* file)
{
    ...
    {
        vt_safe_string::safe_format(str, STRLENGTH, "Unknown archive file version %d", version);
        ReportError(str);
        return 1;
    }
    ...
    if (count < 10000 && error == 0)
    {
        for (i = 0; i < count; ++i)
        {
            if (df.end_of_file)
            {
                vt_safe_string::safe_format(str, STRLENGTH, "Unexpected end of drawers in archive (%d of %d so far)",
                        i + 1, count);
                ReportError(str);
    ...
    {  // Read Exceptions
        ...
        if (error)
        {
            vt_safe_string::safe_format(str, STRLENGTH, "Error %d loading exception data (version %d) from archive",
...
archive_read_error:
    ...
    loaded = 1;
    corrupt = 1;
    vt_safe_string::safe_format(str, STRLENGTH, "Archive '%s' (version %d) invalid", filename.Value(), version);
    ReportError(str);
    changed = 0;
    Unload();
    return 1;
