...
char* Settings::TenderName(int tender_type, int tender_id, genericChar* str)
{
    ...
    static const genericChar* name[] = {
        ...
        "ATM/Debit Card", "Credit Card Tip", "Expenses", "Cash", "Credit Card Fee", "Credit Card Fee", "Debit Card Fee", "Debit Card Fee", nullptr};
    static int value[] = {
        ...
        TENDER_CHARGED_TIP, TENDER_EXPENSE, TENDER_CASH_AVAIL, TENDER_CREDIT_CARD_FEE_DOLLAR, TENDER_CREDIT_CARD_FEE_PERCENT, TENDER_DEBIT_CARD_FEE_DOLLAR, TENDER_DEBIT_CARD_FEE_PERCENT, -1};
    const char* temp;
...
int Settings::Add(PrinterInfo *pr)
{
    ...
    int rv = printer_list.AddToTail(pr);
#ifdef HW_ZONE_DEBUG_MAP
...
int Settings::TermReport(Terminal *t, Report *r)
{
    ...
    {
#ifdef HW_ZONE_DEBUG_MAP
        printf("HWDBG: TermReport idx=%d ti=%p name=%s host=%s\n", idx, (void*)ti, ti->name.Value(), ti->display_host.Value());
        idx++;
#endif
...
int Settings::PrinterReport(Terminal *t, Report *r)
{
    ...
    {
#ifdef HW_ZONE_DEBUG_MAP
        printf("HWDBG: PrinterReport idx=%d pi=%p name=%s host=%s\n", pidx, (void*)pi, pi->name.Value(), pi->host.Value());
        pidx++;
#endif
