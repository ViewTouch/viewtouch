...
Check *Check::Copy(Settings *settings)
{
    ...
    auto newcheck = std::make_unique<Check>();
    if (!newcheck)
        return nullptr;
                          ...
                          ReportZone *rzone, Printer *printer)
{
    ...
    if (employee && employee->training)
    {
        ...
        report->NewLine();
        report->Mode(PRINT_LARGE);
        report->TextL(GlobalTranslate(" ** TRAINING **"), COLOR_RED);
        report->Mode(0);
        report->NewLine();
        report->TextL(GlobalTranslate("Do NOT Prepare This Order"), COLOR_RED);
        report->NewLine();
        report->TextL(" ", COLOR_RED);
        report->Underline(pwidth, COLOR_DEFAULT, ALIGN_CENTER, 0.0);
        report->NewLine();
...
// Helper function to get modifier color based on main order color
static int GetModifierColor(int order_color)
{
    switch (order_color) {
        case COLOR_YELLOW:   return COLOR_ORANGE;     // Darker yellow variant
        case COLOR_RED:      return COLOR_DK_RED;     // Dark red
        case COLOR_GREEN:    return COLOR_DK_GREEN;   // Dark green
        case COLOR_BLUE:     return COLOR_DK_BLUE;    // Dark blue
        case COLOR_PURPLE:   return COLOR_DK_MAGENTA; // Dark purple variant
        case COLOR_TEAL:     return COLOR_DK_TEAL;    // Dark teal
        case COLOR_MAGENTA:  return COLOR_DK_MAGENTA; // Dark magenta
        case COLOR_BROWN:    return COLOR_DK_RED;     // Darker brown
        case COLOR_ORANGE:   return COLOR_REDORANGE;  // Darker orange
        case COLOR_REDORANGE:return COLOR_BROWN;      // Even darker
        case COLOR_SEAGREEN: return COLOR_DK_TEAL;    // Darker sea green
        case COLOR_LT_BLUE:  return COLOR_BLUE;       // Darker light blue
        case COLOR_DK_RED:   return COLOR_BLACK;      // Very dark for already dark red
        case COLOR_DK_GREEN: return COLOR_BLACK;      // Very dark for already dark green
        case COLOR_DK_BLUE:  return COLOR_BLACK;      // Very dark for already dark blue
        case COLOR_DK_TEAL:  return COLOR_BLACK;      // Very dark for already dark teal
        case COLOR_DK_MAGENTA: return COLOR_BLACK;    // Very dark for already dark magenta
        default:             return COLOR_GRAY;       // Fallback for unknown colors
    }
}
    ...
    for (SubCheck *sc = SubList(); sc != nullptr; sc = sc->next)
    {
        ...
        for (Order *order = sc->OrderList(); order != nullptr; order = order->next)
		{
                ...
                (order_target == video_target))
            {
                ...
                // Always set modifier color to be different from order color for visual distinction
                if (order_color == COLOR_DEFAULT)
                {
                    // For normal orders, use red for modifiers
                    modifier_color = COLOR_RED;
                }
                else
                {
                    // For alerting orders, use the mapped modifier color
...
int Check::IsEmpty() const
{
    FnTrace("Check::IsEmpty()");
    for (const SubCheck *sc = SubList(); sc != nullptr; sc = sc->next)
    {
...
int SubCheck::Read(Settings *settings, InputDataFile &infile, int version)
{
    ...
    if (count < 10000 && error == 0)
    {
        for (i = 0; i < count; ++i)
        {
            if (infile.end_of_file)
            {
                ReportError(GlobalTranslate("Unexpected end of orders in SubCheck"));
                return 1; // Error
            }
    ...
    if (count < 10000 && error == 0)
    {
        for (i = 0; i < count; ++i)
        {
            if (infile.end_of_file)
            {
                ReportError(GlobalTranslate("Unexpected end of payments in SubCheck"));
                return 1; // Error
            }
    
            auto pmnt = std::make_unique<Payment>();
    ...
    if (error == 0)
        ...
        ReportError(GlobalTranslate("Error in reading subcheck"));
    return error;
}
...
int SubCheck::PrintReceipt(Terminal *term, Check *check, Printer *printer, Drawer *drawer, int open_drawer)
{
    ...
    if (printer == nullptr)
    {
        ReportError(GlobalTranslate("No printer to print receipt"));
        return 1;  // no printer
    }
    ...
    if (e && e->training)
    {
        ...
        printer->NewLine();
        printer->Write(GlobalTranslate(" ** TRAINING **"), PRINT_LARGE);
        printer->Write(GlobalTranslate("   This Is NOT A Valid Receipt"));
        printer->Write("                                 ", PRINT_UNDERLINE | PRINT_RED);
        printer->NewLine();
    }
